...
import { canConvertFiles, getUpgradeMessage } from '@/backend/utils/planLimits';
...
export const POST = withRateLimit(
  withAuth(async (req, ctx) => {
    try {
      ...
      if (!canConvertFiles(ctx.plan, data.fileCount ?? 1)) {
        return Response.json({ error: getUpgradeMessage('file_count') }, { status: 403 });
      }
      ...
      logger.info(`[API] Created conversion job ${jobRecord.id} for user ${ctx.userId}`);
